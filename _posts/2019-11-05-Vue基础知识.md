...
#### 14. Vue组件之间的传值？
**1.父传子**<br>
<font color="red">子组件通过`props属性`接收从父组件传过来的值，父组件传值的时候使用v-bind将子组件中预留的变量名绑定为data里面的数据</font><br>
子组件：<br>
```html
<template>
...
```
**2.子传父**<br>
<font color="red">首先子组件中用按钮来触发`setData`事件，在`setData`中用`this.$emit`来遍历父组件中`getData`事件，最后返回`this.msg`</font><br>
子组件：<br>
```html
<template>
  <input type="text" v-model="msg">
