...
### 11. window.onload和$(document).ready()的区别
- `window.onload=function(){}`方法是在网页中所有的元素（包括元素的所有关联文件）完全加载到浏览器后才执行的
- `$(document).ready()`可以在DOM载入就绪时就对其进行操纵，并调用执行绑定的函数

### 12. JSON与JS转换
- `JSON.parse()`: JSON字符串转换为JS对象
- `JSON.stringify()`: JS对象转换成JSON字符串

### 13. [JS的执行机制](https://www.cnblogs.com/intangible/p/8066979.html)

### 14. [深拷贝与浅拷贝](https://www.cnblogs.com/echolun/p/7889848.html)
