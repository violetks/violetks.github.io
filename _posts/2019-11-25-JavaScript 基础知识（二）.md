...
（4）构造函数中的 this 是一个隐式对象，类似一个初始化的类型，所有方法和属性都挂载到了这个隐式对象身上，后续通过 new 关键字来调用，从而实现实例化。<br>
#### 改变 this 指向的方法：
- 使用箭头函数<br>
- 在函数内部使用`_this=this;`<br>
- 使用 call、apply、bind<br>
...
### 四、什么是 window 对象？什么是 document 对象？
...
### 十、window.onload 和 $(document).ready() 的区别
...
### 十二、[JS 的执行机制](https://www.cnblogs.com/intangible/p/8066979.html)（★）
（1）JavaScript 是一门**单线程**语言，是按照语句出现的顺序执行。<br>
（2）JavaScript 事件循环<br>
分为：**同步任务**、**异步任务**<br>
